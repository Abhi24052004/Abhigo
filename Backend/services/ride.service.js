...
async function getFare(pickup, destination, event) {
  ...
  if (event === "false") {
    ...
    const fare = Math.round(
      baseFare +
        (distanceTime.distance.value / 1000) * perKmRate +
        (distanceTime.duration.value / 60) * perMinuteRate
    );
    return fare;
  }
}
...
module.exports.startEventRide = async ({ eventId, captainId, userId }) => {
  ...
  if (!ride.eventDateTime)
    throw new Error("Ride is not marked as an event ride");
...
module.exports.paymentsforride = async ({ rideId, paymentmode, paymentID }) => {
  try {
    const updatedRide = await rideModel.findById(rideId);

    (updatedRide.paymentmode = paymentmode),
      (updatedRide.paymentID =
        paymentmode === "online" ? paymentID : `CASH_${rideId}_${Date.now()}`),
      await updatedRide.save();
    return updatedRide;
  } catch (e) {
    throw new Error("Error in updating ride payment status");
  }
};
